...
function stepFence(line: string, current: DocumentFence): { fence: DocumentFence; fenceLine: boolean } {
  ...
  if (!current) {
    if (sequence[0] === '`' && match[2]!.includes('`')) return { fence: null, fenceLine: false };
    return { fence: { marker: sequence[0]!, length: sequence.length }, fenceLine: true };
  }
  const closes = sequence[0] === current.marker && sequence.length >= current.length && !match[2]!.trim();
...
function headingOffsets(content: string): number[] {
  ...
  for (const line of content.split("\n")) {
    const step = stepFence(line, fenced);
    fenced = step.fence;
    if (!step.fenceLine && !fenced && /^## (?!#)/.test(line)) offsets.push(offset);
